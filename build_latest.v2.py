...
WEEK_HEADER_RE = re.compile(r"Semaine\s+du\s*(\d{1,2})\s*[–-]?\s*(\d{1,2})?\s*([A-Za-zÀ-ÿ]+)?", re.I)
...
def parse_erakys_listing_html(html: str, source: Dict[str, str], today: dt.date) -> List[Movie]:
    ...
    for row in soup.select("div.row.fiche-film-div"):
        ...
        movie = movies.setdefault(slug_key(title), Movie(title=title, genre=genre, duration=duration, poster=poster, info_url=info_url, trailer_url=trailer_url, shows=[]))
        ...
        # Correct Erakys parsing: .version-seance blocks are nested, followed by sibling .ligne_ach1
        for version_node in div_horaire.select('.version-seance'):
            version = normalize_space(version_node.select_one('.b-seance').get_text(' ', strip=True) if version_node.select_one('.b-seance') else version_node.get_text(' ', strip=True))
...
def merge_movies(movie_lists: Iterable[List[Movie]]) -> List[Movie]:
    ...
    for movies in movie_lists:
        for movie in movies:
            ...
            if not target:
                merged[key] = Movie(title=movie.title, genre=movie.genre, duration=movie.duration, poster=movie.poster, info_url=movie.info_url, trailer_url=movie.trailer_url, shows=list(movie.shows))
                continue
...
def load_seed_movies(seed_path: Optional[Path], today: dt.date) -> List[Movie]:
    ...
    for wk in data.get('weeks') or []:
        for m in wk.get('movies', []):
            movie = Movie(title=m.get('title',''), genre=m.get('genre',''), duration=m.get('duration',''), poster=m.get('poster',''), info_url=m.get('infoUrl',''), trailer_url=m.get('trailerUrl',''), shows=[])
            for day in wk.get('week', {}).get('days', []):
                ...
                for item in (m.get('days') or {}).get(key, []):
                    movie.shows.append(Show(date=d, time=item.get('time',''), version=item.get('version',''), cinema=item.get('cinema',''), booking_url=item.get('bookingUrl','')))
            if movie.shows:
                out.append(movie)
...
def main() -> int:
    ...
    for source in ERAKYS_SOURCES:
        ...
        if overrides.get(source['slug']):
            html = Path(overrides[source['slug']]).read_text(encoding='utf-8', errors='replace')
        else:
            try:
                html = fetch_text(sess, source['url'])
            except Exception as exc:
                print(f"[warn] failed source {source['name']}: {exc}", file=sys.stderr)
                continue
        movies = parse_erakys_listing_html(html, source, today)
